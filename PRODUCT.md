...
The page title is `Edit Multiple Mix`, left-aligned above a two-column desktop layout. `Basic Info` and `Thumbnail info` use equal-width columns, with Thumbnail info on the right. Before content selection, both sections show `Need to select content first`; Basic Info exposes `Select Content`, while `Edit Thumbnail` remains visible and responds with the toast `Please select content first` instead of opening the editor. Select Content opens a modal with Type, Category, and Style selectors plus a scrollable Style Configuration preview. Cancel or Escape closes it without changing the page; Save adds the selected content. After saving, Basic Info shows the required `Style Name` input, while Thumbnail info reveals Default Set and country-set actions. The left navigation can collapse to a compact icon-oriented rail and expand again; narrow screens stack the page content into one column.

A large fixed `Show Tip` button starts an optional eight-step guided walkthrough from the beginning every time. Each step spotlights the required control or work area while four semi-transparent black blockers dim and disable all nonessential regions. The walkthrough follows Select Content, Save, Edit Thumbnail, thumbnail-field review, reference-video upload, Auto Resize, completion review, and Confirm. In the reference-video step, clicking the highlighted Video Thumbnail area automatically uploads the bundled `test.mov` without opening the system file picker. Selecting Auto Resize generates all thumbnail fields, then the next step spotlights the full thumbnail area and confirms completion. Action steps continue when the highlighted control is used, informational steps provide Next, and the card closes from an upper-right X or Escape.
